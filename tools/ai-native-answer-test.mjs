...
assert.match(js, /const AI_ANSWER_CARD_LIMIT = 4/, 'AI answers must cap metric cards for a focused first screen');
assert.match(js, /const AI_ANSWER_SECTION_LIMIT = 2/, 'AI answers must cap explanatory sections for a focused first screen');
assert.match(js, /const AI_ANSWER_SECTION_ROW_LIMIT = 3/, 'AI answers must cap rows inside explanatory sections');
assert.match(js, /const AI_ANSWER_ACTION_LIMIT = 3/, 'AI answers must cap visible actions to avoid clutter');
assert.match(js, /const AI_ANSWER_EVIDENCE_LIMIT = 3/, 'AI answers must cap visible evidence while preserving traceability');
...
assert.match(js, /可核对来源/, 'AI evidence block must describe sources as checkable records');
assert.match(js, /另外还有 \$\{escapeHtml\(hiddenEvidenceCount\)\} 条来源，打开详情可核对。/, 'AI answer renderer must tell users when more source records exist');
