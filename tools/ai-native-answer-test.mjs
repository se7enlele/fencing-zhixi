...
assert.match(js, /function competitionCoverageStageCards\(competition\)[\s\S]*\['赛事记录', competition\?\.sportName \? '找到相近' : '未找到'\][\s\S]*\['项目名单', coverage\.hasProject \? '可查看' : '先核对赛事'\][\s\S]*\['报名名单', coverage\.hasRoster \? '可查看' : coverage\.roster\][\s\S]*\['赛果成绩', coverage\.score\]/, 'AI similar-competition fallback must expose event, project, roster and score coverage separately');
