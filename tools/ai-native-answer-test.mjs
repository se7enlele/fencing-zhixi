...
assert.match(js, /const hasStatusListIntent = Boolean\(status\)/, 'AI competition stats should allow status-based event-list questions');
assert.match(js, /if \(!hasStatsIntent && !hasStatusListIntent\) return null;/, 'specific event-name queries must not be routed to regional competition stats unless they ask for a status list');
...
assert.match(js, /function detectCompetitionCoverageQuestion\(query = ''\)/, 'AI must detect user questions about missing competition data');
assert.match(js, /function competitionCoverageNameHint\(query = ''\)/, 'AI missing-data diagnosis must strip database wording before matching event names');
assert.match(js, /const competitionCoverageQuestion = detectCompetitionCoverageQuestion\(text\);[\s\S]*buildAiCompetitionCoverageReport\(text, competitionCoverageQuestion\);[\s\S]*const competition = detectCompetitionInQuery\(text\);/, 'AI missing-data diagnosis must run before plain competition lookup');
assert.match(js, /function relatedCompetitionsForQuery\(query\)[\s\S]*const coverageHint = competitionCoverageNameHint\(query\);[\s\S]*const seed = coverageHint\.length >= 4 \? coverageHint : query;[\s\S]*withoutYearAlias\(seed\)[\s\S]*chineseAdminAlias\(seed\)[\s\S]*competitionNameAliasTerms\(competition\)/, 'AI missing competition fallback must clean missing-data wording before matching related event suggestions');
...
assert.match(js, /function competitionCoverageStageCards\(competition\)[\s\S]*\['赛事记录', competition\?\.sportName \? '找到相近记录' : '未完全命中'\][\s\S]*\['项目名单', coverage\.hasProject \? '可查看' : '确认后查看'\][\s\S]*\['报名名单', coverage\.hasRoster \? '可查看' : coverage\.roster\][\s\S]*\['赛果成绩', coverage\.score\]/, 'AI similar-competition fallback must expose event, project, roster and score coverage separately');
assert.match(js, /function competitionCoverageDiagnosisRows\(competition\)[\s\S]*项目名单[\s\S]*报名名单[\s\S]*赛果成绩/, 'AI similar-competition fallback must explain which coverage layers can be opened');
...
assert.match(js, /function buildAiCompetitionCoverageReport\(query, competitionLike\)/, 'AI must build a coverage diagnosis for missing-data questions');
assert.match(js, /const cards = nearest[\s\S]*\? competitionCoverageStageCards\(nearest\)[\s\S]*: missingCompetitionCoverageCards\(competitionLike, relatedCompetitions\)/, 'AI coverage diagnosis must use the same four-layer card model for similar competitions');
assert.match(js, /\['赛事记录', '未完全命中'\][\s\S]*\['项目名单', '确认后查看'\][\s\S]*\['报名名单', '确认后查看'\][\s\S]*\['赛果成绩', '确认后查看'\]/, 'missing competition cards must separate event, project, roster and result coverage with actionable copy');
assert.match(js, /项目名单：确认赛事名称后查看对应项目。[\s\S]*报名名单：确认赛事名称后查看报名情况。[\s\S]*赛果成绩：确认赛事名称后查看成绩和对阵。/, 'missing competition coverage rows must tell users what to do next');
...
assert.match(js, /没有找到\$\{competitionLike\.year\}年完全一致赛事/, 'AI missing-competition fallback must say when the requested event record is unavailable');
...
assert.match(js, /来自你的提问：\$\{state\.aiCompetitionFilterQuestion\}/, 'AI filtered competition lists must show which user question created the filter');
...
assert.match(js, /return parts\.length \? parts\.join\(' · '\) : '';/, 'AI competition filter summary must keep the range copy clean');
...
assert.match(js, /class="ai-filter-chip-row"[\s\S]*相关条件/, 'AI filtered competition lists must show active filter conditions');
