...
assert.doesNotMatch(js, /试用包含|申请产品试用|落地这类报告服务|商业转化|下一步投入|沉淀为提醒/, 'AI conversion card must use customer actions instead of internal product-validation wording');
assert.match(js, /可以继续查看/, 'AI conversion card should frame follow-up content as a user-visible continuation');
...
assert.doesNotMatch(js, /暂未找到同项目近期赛事/, 'prematch copy should offer a next action instead of a dead-end no-match message');
...
assert.match(js, /为你找到：/, 'AI-to-database filter context should use user-facing wording');
